...
population_data_test <- read_csv("https://www.opendata.nhs.scot/dataset/7f010430-6ce1-4813-b25c-f7f335bdc4dc/resource/27a72cc8-d6d8-430c-8b4f-3109a9ceadb1/download/hb2019_pop_est_03102025.csv") %>% 
  clean_names() %>% 
...
graph_per_100k <- data_cgm %>% 
  ...
  group_by(paid_date_month, hb_name, hb_population, hbt) %>% 
...
data_cgm %>% 
  left_join(hb_names, join_by(hbt == hb)) %>% 
  left_join(population_data, join_by(hb_name)) %>% 
  left_join(hb_map, join_by(hbt == hb_code)) %>% 
  group_by(year(paid_date_month), hb_name, hb_population) %>% 
  summarise(total = sum(paid_quantity),
            total_per_100k = unique((total/hb_population)*100000)) %>% 
  filter(!is.na(hb_name))
  filter(year(paid_date_month) == 2023) %>%
