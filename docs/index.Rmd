...
summary_function <- function(grouping, filtering){data_cgm %>% 
    left_join(hb_names, join_by(hbt == hb)) %>% #add to intial data cleaning
...
graph_per_100k <- summary_function(
  grouping = c("paid_date_month", "hb_name", "hbt", "year"), 
  filtering = hb_name)
```
