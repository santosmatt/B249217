...
data_cgm <- data_cgm %>% 
  mutate(hbt = recode(hbt, "S08000018" = "S08000029", "S08000021" = "S08000031", "S08000023" = "S08000032", "S08000027" = "S08000030"), 
         year = as.numeric(year(paid_date_month))) #done to allow joinin
```

# Results
```{r creating-function-summary}
summary_function <- function(data, grouping, filtering){data%>% 
    left_join(hb_names, join_by(hbt == hb)) %>% #add to initial data cleaning
...
cgm_map <- cgm_map_data %>%
  ...
  geom_sf_interactive(
    aes(data_id = hb_name, 
        tooltip = paste(hb_name, "\n", "CGM prescriptions per diabetic patient", round(cgm_per_diabetic_patient, 2)))) +
...
age_table_data <- summary_function(data = data_cgm, grouping = c("hb_name", "year", "hbt"), filtering = hb_name) %>% 
...
#   left_join(gp_practice_data, join_by(gp_practice == practice_code)) %>% 
#   left_join(simd_data, join_by(data_zone, hb)) %>% 
#   summary_function(data = ., grouping = c("simd_decile", "paid_date_month", "hbt", "year"),
#                    filtering = gp_practice_name) %>% 
#   left_join(gp_population_by_simd, join_by(simd_decile)) %>% 
#   mutate(total_per_100k = (total_cgm/total_gp_population)*100000)
#   
```
