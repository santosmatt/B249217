...
population_data_raw <- read_csv("https://www.opendata.nhs.scot/dataset/7f010430-6ce1-4813-b25c-f7f335bdc4dc/resource/27a72cc8-d6d8-430c-8b4f-3109a9ceadb1/download/hb2019_pop_est_03102025.csv") %>% 
  clean_names() 
```
...
diabetes_by_healthboard <- read.csv(here("data","diabetes_rates_by_healthboard.csv")) %>%
  clean_names() %>% 
  filter(!is.na(hb_name)) #removing empty columns
```
...
graph_per_100k <- data_cgm %>% 
  ...
  group_by(paid_date_month, hb_name, hbt, year) %>% 
...
#Filtering data for CGM map by joing with needed databases and calculating totals
cgm_map_data <- data_cgm %>% 
...
age_table_data <- data_cgm %>% 
  left_join(hb_names, join_by(hbt == hb)) %>% 
  group_by(hb_name, hbt, year) %>% 
  summarise(total_cgm = sum(paid_quantity)) %>% 
  left_join(population_data, join_by(hbt == hb, year == year)) %>% 
  mutate(total_per_100k = (total_cgm/all_ages)*100000,
         across(age_under_16:age_65_plus, ~.x/all_ages)) %>% 
  filter(!is.na(hb_name))
```
